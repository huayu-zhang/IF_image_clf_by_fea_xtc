...
create.filelist <- function(folder){
  ...
  fileList <- list(
    "R" = files$link[files$channel == "red"],
    "G" = files$link[files$channel == "green"],
    "B" = files$link[files$channel == "blue"],
    "Y" = files$link[files$channel == "yellow"]
  )
...
img.Import_512 <- function(index, fileList)
{
  return(
...
# Create file lists -------------------------------------------------------
paths <- list(
  path_Train = "E:/trainMerge/",
  path_Test = "E:/testMerge/",
  
  path_Train_512 = "E:/data_camp512/raw/train/", 
  path_Test_512 = "E:/data_camp512/raw/test/"
)


files <- list(
  trainFiles = list(
    "RGB" = list.files(path = paste(paths$path_Train, "RGB", sep = ""), full.names = T),
    "YGB" = list.files(path = paste(paths$path_Train, "YGB", sep = ""), full.names = T)
  ),
  
  
  testFiles = list(
    "RGB" = list.files(path = paste(paths$path_Test, "RGB", sep = ""), full.names = T),
    "YGB" = list.files(path = paste(paths$path_Test, "YGB", sep = ""), full.names = T)
  ),
  
  trainFiles_512 = create.filelist(paths$path_Train_512),
  testFiles_512 = create.filelist(paths$path_Test_512)
)


# Cutoff for bin channels -----------------------------------------------------

binChannel <- function(channel)
{
  step <- c()
...
featurePipe <- function(index)
{
  ...
  if (!file.exists(paste("E:/trainOutput/",  index, "_",sampleName, ".rdata", sep = "")))
  {
    write(x = proc.time(), file = paste("E:/trainLog/", index, sampleName, "_start", ".txt", sep = ""))
    ...
    if (sum(img_bin_512$B) < 500) 
    {
      save(list = c("img_bin_512"), file = paste("E:/trainOutput/", index, "_", sampleName, ".rdata", sep = ""))
      return()
    }
    ...
    save(list = c("img_bin_512", "img_masks_512", "img_masks_G.Huayu", "img_masks_G.Otsu", "featureList"), file = paste("E:/trainOutput/", index, "_", sampleName, ".rdata", sep = ""))
    
    if (file.exists(paste("E:/trainOutput/", sampleName, ".rdata", sep = ""))) {
      write(x = proc.time(), file = paste("E:/trainLog/", index,  sampleName, "_finish", ".txt", sep = ""))
    } else {write(x = proc.time(), file = paste("E:/trainLog/", index, sampleName, "_error", ".txt", sep = ""))}
  } 
...
featurePipe.Test <- function(index)
{
  
  sampleName <- strsplit(x = files$testFiles$RGB[index], split = "\\.|\\_|\\/")[[1]][4]
  
  if (!file.exists(paste("E:/testOutput/",  index, "_",sampleName, ".rdata", sep = "")))
  {
    write(x = proc.time(), file = paste("E:/testLog/", index, sampleName, "_start", ".txt", sep = ""))
    
    # Input -------------------------------------------------------------------
    
    img <- img.Import(index = index, fileList = files$testFiles)
    img_512 <- img.Import_512(index = index, fileList = files$testFiles_512)
    ...
    if (sum(img_bin_512$B) < 500) 
    {
      save(list = c("img_bin_512"), file = paste("E:/testOutput/", index, "_", sampleName, ".rdata", sep = ""))
      return()
    }
    ...
    save(list = c("img_bin_512", "img_masks_512", "img_masks_G.Huayu", "img_masks_G.Otsu", "featureList"), file = paste("E:/testOutput/", index, "_", sampleName, ".rdata", sep = ""))
    
    if (file.exists(paste("E:/testOutput/", sampleName, ".rdata", sep = ""))) {
      write(x = proc.time(), file = paste("E:/testLog/", index,  sampleName, "_finish", ".txt", sep = ""))
    } else {write(x = proc.time(), file = paste("E:/testLog/", index, sampleName, "_error", ".txt", sep = ""))}
  } 
